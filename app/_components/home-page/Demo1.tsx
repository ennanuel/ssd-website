...
export default function AppDemo({ goToNextSlide, carouselTimeout, duration }: CarouselProps) {
    ...
    return (
        <HeroAppDemoContainer>
            <div className="w-fit sm:w-full grid grid-cols-1 grid-rows-2 p-4 sm:p-6 md:p-8 gap-4 md:gap-6">
                ...
                <div className="flex lg:grid lg:grid-rows-1 lg:grid-cols-[repeat(3,_auto)] gap-4 md:gap-6">
                    {
                        CAMERAS.slice(3, 6).map(({ id, name, cameraFootage }) => (
                            <div key={id} className="h-full aspect-square sm:aspect-auto lg:aspect-square flex flex-col gap-3 overflow-hidden rounded-xl p-2 bg-white shadow-xl shadow-black/20">
                                <span className="relative flex-1">
                                    <Image src={cameraFootage} fill alt="Camera footage" className="w-full h-full object-cover block rounded-md" />
                                </span>
                                <span className="bg-white flex flex-col gap-2">
