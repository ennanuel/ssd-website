...
const STEPS = [
    {
        ...
        colors: "bg-white/5 text-white",
        iconColors: "bg-blue-500 text-white"
    },
    {
        title: "Configuration",
    ...
    },
    {
        ...
        colors: "bg-yellow-300 text-black",
        iconColors: "bg-black text-yellow-300"
    }
];
        ...
        <section className="min-h-[calc(100vh_-_var(--header-large-height))] bg-dark-blue px-4 xs:px-6 sm:px-10">
            <div className="mx-auto max-w-[var(--max-width)] pt-40 pb-20">
                ...
                <ul className="mt-14 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    {
                        STEPS.map(({ title, description, colors, iconColors }, index) => (
                            <li key={title} className={`${colors} relative flex gap-4 p-4 pb-8 border border-white/20 rounded-2xl`}>
                                <div className="">
                                    <span className={`${iconColors} flex items-center justify-center w-10 aspect-square rounded-full`}>{index + 1}</span>
                                </div>
