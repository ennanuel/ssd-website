...
export default function Industries() {
    ...
    return (
        <div className="bg-blue-300/10">
            <section className="bg-dark-blue text-white relative">
                <div className="relative mx-auto max-w-[var(--max-width)] pt-30 pb-40">
                    <div className="flex flex-col items-center justify-center">
                        <h2 className="text-5xl leading-[3.8rem] max-w-[32ch] text-center">Industries we serve</h2>
                        <p className="mt-4 text-lg leading-[1.8rem] max-w-[64ch] text-center">Safe Security Dynamics provides advanced, AI-driven license plate recognition solutions to a variety of industries, including law enforcement.</p>
                    </div>
                </div>
            </section>
            <section className="relative">
                <div className="mx-auto max-w-[var(--max-width)] min-h-screen -mt-20 pb-20">
                    <ul className="mx-auto max-w-[var(--max-width)] min-h-screen grid grid-cols-3 gap-6">
                        {
                            INDUSTRIES.map(({ title, desc, href, Icon }) => (
                                <li key={title}>
                                    <Link href={`/industries${href}`} className="h-full p-8 flex flex-col items-center justify-center rounded-2xl bg-white border border-gray-200 hover:border-blue-200 hover:-translate-y-4">
                                        <span className="w-16 aspect-square flex items-center justify-center rounded-full bg-blue-400/20">
                                            <Icon size={32} className="text-blue-600" />
                                        </span>
                                        <h3 className="mt-10 text-xl font-semibold text-blue-600 text-center">{title}</h3>
